...
// .wrangler/tmp/bundle-wITFxq/strip-cf-connecting-ip-header.js
...
async function getKV(env2, key, def = null) {
  try {
    const val = await env2.RANKING_KV.get(key, "json");
...
__name(getKV, "getKV");
async function setKV(env2, key, val) {
  await env2.RANKING_KV.put(key, JSON.stringify(val));
}
__name(setKV, "setKV");
...
var DEFAULT_STUDENTS = ["\u5305\u987A\u901A", "\u90D1\u6E05\u98A2", "\u90B9\u4EC1\u6CFD", "\u79E6\u97F5\u8D3A", "\u8D75\u5DCD\u9716", "\u738B\u6668\u65ED", "\u5F20\u5E7F\u6D9B", "\u7A0B\u6CD3\u535A", "\u59DC\u4E59\u4E30", "\u5510\u4F73\u5947", "\u8D75\u6893\u6E24", "\u5434\u4FCA\u9716", "\u738B\u8BD1\u5E7F"];
...
async function ensureInitialized(env2) {
  const existingStudents = await getKV(env2, "students");
  ...
  await setKV(env2, "users", users);
  await setKV(env2, "students", DEFAULT_STUDENTS);
  const scores = {};
  DEFAULT_STUDENTS.forEach((s) => scores[s] = 15);
  await setKV(env2, "scores", scores);
  await setKV(env2, "logs", []);
  await setKV(env2, "calendar", DEFAULT_CALENDAR);
  await setKV(env2, "phoneOptOuts", {});
  await setKV(env2, "todayDeductions", {});
  await setKV(env2, "punishData", { punishRecords: {}, maxDoneLevel: {} });
  await setKV(env2, "_initialized", "yes");
}
...
__name(superAdminOnly, "superAdminOnly");
...
async function addLog(env2, user, action, detail) {
  const logs = await getKV(env2, "logs", []);
  logs.unshift({
...
function isDeductionDay(dateStr, calendar) {
  ...
  if (calendar.excludedWeekdays.includes(wd) && !calendar.customInclusions.includes(dateStr))
    return false;
  if (calendar.customExclusions.includes(dateStr))
    return false;
  for (const p of calendar.periods) {
    const y = d.getFullYear();
    const s = /* @__PURE__ */ new Date(p.skipYearBoundary ? `${y - 1}-${p.start}T12:00:00` : `${y}-${p.start}T12:00:00`);
    const e = /* @__PURE__ */ new Date(`${y}-${p.end}T12:00:00`);
    if (p.skipYearBoundary && d.getMonth() + 1 < 6) {
      const s2 = /* @__PURE__ */ new Date(`${y - 1}-${p.start}T12:00:00`);
      const e2 = /* @__PURE__ */ new Date(`${y}-${p.end}T12:00:00`);
      if (d >= s2 && d <= e2)
        return false;
    } else {
      if (d >= s && d <= e)
...
__name(isDeductionDay, "isDeductionDay");
...
app.post("/api/students", authMiddleware, superAdminOnly, async (c) => {
  ...
  if (students.includes(name.trim()))
    return c.json({ error: "\u8BE5\u5B66\u751F\u5DF2\u5B58\u5728" }, 400);
...
app.get("/api/deductions/today", async (c) => {
  const today = getDateStr(/* @__PURE__ */ new Date());
...
function checkPeriodExclusion(dateStr, calendar) {
  ...
  for (const p of calendar.periods) {
    ...
    let s, e;
    if (p.skipYearBoundary && d.getMonth() + 1 < 6) {
      s = /* @__PURE__ */ new Date(`${y - 1}-${p.start}T12:00:00`);
      e = /* @__PURE__ */ new Date(`${y}-${p.end}T12:00:00`);
    } else {
      s = /* @__PURE__ */ new Date(`${y}-${p.start}T12:00:00`);
      e = /* @__PURE__ */ new Date(`${y}-${p.end}T12:00:00`);
    }
    if (d >= s && d <= e)
      return p.label;
...
__name(checkPeriodExclusion, "checkPeriodExclusion");
...
app.post("/api/phone-opt-out", authMiddleware, adminOnly, async (c) => {
  ...
  if (!["noon", "evening"].includes(slot))
    return c.json({ error: "\u65E0\u6548\u65F6\u6BB5" }, 400);
  const targetDate = date || getDateStr(/* @__PURE__ */ new Date());
  const now = /* @__PURE__ */ new Date();
  ...
  const phoneOptOuts = await getKV(c.env, "phoneOptOuts", {});
  if (!phoneOptOuts[targetDate])
    phoneOptOuts[targetDate] = { noon: [], evening: [] };
...
app.get("/api/admins", authMiddleware, superAdminOnly, async (c) => {
  const users = await getKV(c.env, "users", {});
  const admins = Object.entries(users).filter(([_, u]) => u.role === "admin").map(([username, u]) => ({ username, name: u.name }));
...
async function handleScheduled(event, env2, ctx) {
  ...
  const slot = currentHour === 12 && currentMin < 1 ? "noon" : currentHour === 16 && currentMin < 46 ? "evening" : null;
  if (!slot)
    return;
  const calendar = await getKV(env2, "calendar", DEFAULT_CALENDAR);
  if (!isDeductionDay(today, calendar))
    return;
  const phoneOptOuts = await getKV(env2, "phoneOptOuts", {});
  const todayOptOuts = phoneOptOuts[today] || { noon: [], evening: [] };
  const exempted = todayOptOuts[slot] || [];
  const students = await getKV(env2, "students", []);
  const scores = await getKV(env2, "scores", {});
  const todayDeductions = await getKV(env2, "todayDeductions", {});
  if (!todayDeductions[today])
    todayDeductions[today] = { noon: false, evening: false };
  if (todayDeductions[today][slot])
    return;
  const deducted = [];
  ...
  await setKV(env2, "scores", scores);
  await setKV(env2, "todayDeductions", todayDeductions);
...
var worker_default = {
  ...
  async scheduled(event, env2, ctx) {
    await handleScheduled(event, env2, ctx);
  }
};
...
// .wrangler/tmp/bundle-wITFxq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wITFxq/middleware-loader.entry.ts
